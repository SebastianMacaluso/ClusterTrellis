...
class TrellisNode:
    ...
    def compute_map_tree_split(self, a_trellis):
        ...
        if self.is_leaf():
            ...
            self.map_tree_energy = -np.inf +0j
            self.logZ = -np.inf +0j
            ...
            for node in special_nodes:
                ...
                logger.info("self.logZ , partial_logZ = %s , %s", self.logZ, partial_logZ)
                self.logZ = logsumexp(np.asarray([self.logZ, partial_logZ]))

                logger.info("logsumexp = %s",self.logZ)

                self.levelLHweight.append(np.real(partial_logZ))
                ...
                # print("np.absolute(self.map_tree_energy) = ", np.absolute(self.map_tree_energy))
                if np.real(energy) > np.real(self.map_tree_energy):
            ...
            if np.absolute(self.map_tree_energy) == - np.inf:
            ...
            if np.absolute(self.logZ) == -np.inf:
                """Assign 0 probability when Delta_parent is below the threshold, not allowed under the model"""
